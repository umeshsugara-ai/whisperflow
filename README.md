...
- **Cleanup tiers**: `off` = verbatim; `rules` = deterministic filler/punctuation cleanup (default); **`groq` = cloud text polish that reuses the `GROQ_API_KEY` you already set for transcription — zero extra setup, ~0.3s, fixes romanized-Hindi spellings ("nighi"→"nahi") and punctuation without rephrasing (recommended if you use the Groq engine)**; `llm` = local Ollama model (fully private — install [Ollama](https://ollama.com) and `ollama pull qwen2.5:3b-instruct`); `gemini` = cloud polish with your own Gemini key. Cloud polish tiers send transcript TEXT only, never audio. If the polish backend is down/rate-limited, that dictation silently degrades to `rules` — it never blocks.
...
```powershell
python -m pytest tests/ -q          # 275 unit tests
